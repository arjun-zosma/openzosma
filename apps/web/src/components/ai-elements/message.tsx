...
import { ChartBlock } from "@/src/components/ai-elements/chart-block"
...
export const MessageContent = ({ children, className, ...props }: MessageContentProps) => (
	<div
		className={cn(
			...
			"group-[.is-user]:ml-auto group-[.is-user]:rounded-lg group-[.is-user]:bg-secondary group-[.is-user]:px-4 group-[.is-user]:py-3 group-[.is-user]:text-foreground text-right",
			"group-[.is-assistant]:text-foreground group-[.is-assistant]:text-left rounded-lg bg-green-100 dark:group-[.is-assistant]:bg-green-900/80 px-4 py-3",
			className,
		)}
		{...props}
	>
...
/**
 * Custom <pre> renderer that intercepts ```chart code blocks
 * and renders them as interactive recharts visualizations.
 * Mermaid diagrams are handled natively by Streamdown.
 */
const CustomPre = ({ children, ...props }: HTMLAttributes<HTMLPreElement> & { children?: ReactNode }) => {
	// Check if the child is a <code> element with language-chart class
	const child = Children.only(children)
	if (
		isValidElement(child) &&
		(child as ReactElement<{ className?: string; children?: ReactNode }>).props?.className?.includes("language-chart")
	) {
		const raw = String((child as ReactElement<{ className?: string; children?: ReactNode }>).props?.children ?? "")
		return <ChartBlock json={raw} />
	}
	return <pre {...props}>{children}</pre>
}

const messageComponents = { pre: CustomPre }
